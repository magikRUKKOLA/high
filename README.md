...
Most TUI LLM suites today are implemented extremely inefficiently. For example, [`charmbracelet/mods`](https://github.com/charmbracelet/mods/issues/635) can [delay final output by a minute or so](https://github.com/magikRUKKOLA/high/discussions/4). The [`Claude Code` supports max 60 FPS](https://www.youtube.com/watch?v=LvW1HTSLPEk) due to the internal usage of `React` *(and all that it implies)*. etc.
